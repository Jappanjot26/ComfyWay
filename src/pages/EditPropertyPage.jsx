...
const EditPropertyPage = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setImages(
            property.images.concat(Array(3 - property.images.length).fill(""))
          );
          setAvailableDates({
            start: new Date(property.availableDates.start),
            end: new Date(property.availableDates.end),
          });
        } else {
    ...
    fetchProperty();
  }, [id, currentUser.id, currentUser.token, navigate]);
